    ...
    try {
      ...
      if (response.error) {
        ...
        if (code === "INVALID_EMAIL_OR_PASSWORD") {
          ...
          toast.error(
            `Login failed: ${response.error.message || "Please check your network."}`
          );
        }
        return;
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center gap-2">
          <Checkbox
            ...
            onCheckedChange={(v) =>
              setForm((f) => ({ ...f, rememberMe: Boolean(v) }))
            }
        ...
        <Button
          type="submit"
          className="w-full h-12 rounded-xl text-lg font-semibold shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all active:scale-95"
          disabled={loading}
        >
...
// Wrap in Suspense — required by Next.js 13+ when using useSearchParams
